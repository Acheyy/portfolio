...
export function ProjectCard({ project, index, isMobile }: ProjectCardProps) {
  ...
  return (
    ...
    >
      <div className="glow-card flex h-full flex-col p-4 sm:p-6 group">
        <Link
          to="/projects/$slug"
          params={{ slug: project.slug }}
          className="flex min-h-0 flex-1 flex-col rounded-lg focus:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2 focus-visible:ring-offset-bg-card"
        >
        ...
        <div className="mt-auto pt-3 sm:pt-4 flex items-center justify-between gap-2">
          <Link
            to="/projects/$slug"
            params={{ slug: project.slug }}
            className="inline-flex items-center gap-1.5 text-xs font-medium text-accent-light cursor-pointer"
          >
            View Details
